...
public class EnderDragonAxeItem extends AxeItem {
    private static final int COOLDOWN = 300;
    public EnderDragonAxeItem(float bonus, float speed) { super(ToolMaterials.NETHERITE, LNE_WeaponItems.meleeSettings(bonus, speed)); }
    @Override public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (!attacker.getWorld().isClient && attacker instanceof PlayerEntity player
            ...
            LNE_WeaponItems.spawnDragonParticles(attacker.getWorld(), target);
        }
