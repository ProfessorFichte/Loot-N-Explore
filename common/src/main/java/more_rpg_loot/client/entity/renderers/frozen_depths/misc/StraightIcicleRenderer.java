...
/**
 * Renderer for StraightIcicleEntity that animates the icicle emerging and sinking.
 */
public class StraightIcicleRenderer extends EntityRenderer<StraightIcicleEntity> {
    ...
    @Override
    public void render(StraightIcicleEntity entity, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
        float animationProgress = entity.getAnimationProgress(tickDelta);

        if (animationProgress == 0.0F) {
            return; // Don't render before animation starts
        }
        ...
        if (animationProgress < 0.5F) {
            // First half: emerging from ground
            emergeProgress = animationProgress * 2.0F;
        } else {
            // Second half: sinking back down
    ...
    @Override
    public Identifier getTexture(StraightIcicleEntity entity) {
        return TEXTURE;
    }
}
