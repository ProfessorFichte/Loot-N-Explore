...
public class RWA_Items {
    ...
    public static void registerRangedWeaponAPIItems(){
        ...
        ItemGroupEvents.modifyEntriesEvent(Group.RPG_FOOD_KEY).register((content) -> {
            /// T0 BUFF ITEMS
            content.addAfter(CommonItems.SWEET_BERRY_PUNCH.item(),APPLE_JUICE);
            /// T1 BUFF ITEMS
            content.addAfter(CommonItems.MALT_EXTRACT.item(),WALDMEISTER);
            /// T2 BUFF ITEMS
            content.addAfter(CommonItems.ESPRESSO.item(),FORREST_SPIRIT);
            /// T3 BUFF ITEMS
            content.addAfter(CommonItems.THE_UNSHAKABLE.item(),WOODSNAKE_POTION);
        });
    }
}
