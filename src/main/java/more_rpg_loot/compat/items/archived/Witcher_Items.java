...
public class Witcher_Items {
    ...
    public static void registerWitcherItems() {
        ...
        ItemGroupEvents.modifyEntriesEvent(Group.RPG_FOOD_KEY).register((content) -> {
            /// T0 BUFF ITEMS
            content.addAfter(CommonItems.HOT_CHOCOLATE.item(),BEAUCLAIR_WHITE);
            /// T1 BUFF ITEMS
            content.addAfter(CommonItems.MALT_EXTRACT.item(),RIVIAN_KRIEK);
            /// T2 BUFF ITEMS
            content.addAfter(CommonItems.VITAL_DRINK.item(),BUTCHER_OF_BLAVIKEN);
            /// T3 BUFF ITEMS
            content.addAfter(CommonItems.THE_UNSHAKABLE.item(),WHITE_WOLF);
        });
